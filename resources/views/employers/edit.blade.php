<x-navigation>
    ...
    <body>
        <x-slot:heading> Edit Employer </x-slot:heading>
        <p>TODO</p>
        <form action="/employer/update/{{ $employer->id }}" method="POST"
            class="max-w-lg mx-auto bg-white p-6 rounded-2xl shadow-md space-y-6">
            @csrf
            @method('patch');
            <h2 class="text-2xl font-bold text-gray-800 text-center">Edit New employer</h2>

            <div>
                <x-form-label for="name">Name</x-form-label>
                <x-form-input name="name" value="{{ $employer->name }} " />
